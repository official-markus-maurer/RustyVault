...
fn test_sam_output_root_for_origin_mode_uses_source_parent() {
    ...
    .unwrap();
    let dir_root =
        RomVaultApp::sam_output_root_for_source(Path::new("C:\\Input\\SetDir"), "C:\\Output", true)
            .unwrap();
...
fn test_sam_process_7z_item_creates_archive_from_directory_source() {
    ...
    assert_eq!(
        archive.blocks[0].coders[0].encoder_method_id(),
