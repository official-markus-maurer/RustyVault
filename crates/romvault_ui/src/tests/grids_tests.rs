...
fn test_game_summary_bucket_treats_not_collected_as_merged() {
    assert_eq!(
        game_summary_bucket(RepStatus::NotCollected),
        Some(RomStatusBucket::Merged)
    );
    assert_eq!(
        game_summary_bucket(RepStatus::UnNeeded),
        Some(RomStatusBucket::Merged)
    );
    assert_eq!(
        game_summary_bucket(RepStatus::Unknown),
...
fn test_game_row_color_treats_not_collected_like_merged_statuses() {
    assert_eq!(
        game_row_color(RepStatus::NotCollected),
        egui::Color32::from_rgb(60, 60, 60)
    );
    assert_eq!(
        game_row_color(RepStatus::UnNeeded),
        egui::Color32::from_rgb(60, 60, 60)
    );
}
...
fn test_game_row_color_treats_corrupt_can_be_fixed_as_yellow() {
    assert_eq!(
        game_row_color(RepStatus::CorruptCanBeFixed),
...
fn test_game_summary_bucket_treats_deleted_as_fixable() {
    assert_eq!(
        game_summary_bucket(RepStatus::Deleted),
        Some(RomStatusBucket::Fixes)
    );
}
...
fn test_game_summary_bucket_treats_in_to_sort_as_fixable() {
    assert_eq!(
        game_summary_bucket(RepStatus::InToSort),
        Some(RomStatusBucket::Fixes)
    );
}
...
fn test_game_row_color_treats_missing_family_variants_as_red() {
    assert_eq!(
        game_row_color(RepStatus::Corrupt),
        egui::Color32::from_rgb(80, 40, 40)
    );
    assert_eq!(
        game_row_color(RepStatus::DirCorrupt),
        egui::Color32::from_rgb(80, 40, 40)
    );
    assert_eq!(
        game_row_color(RepStatus::Incomplete),
        egui::Color32::from_rgb(80, 40, 40)
    );
}
...
fn test_game_and_rom_row_colors_treat_directory_status_families_consistently() {
    assert_eq!(
        game_row_color(RepStatus::DirCorrect),
        egui::Color32::from_rgb(40, 80, 40)
    );
    assert_eq!(
        game_row_color(RepStatus::DirMissing),
        egui::Color32::from_rgb(80, 40, 40)
    );
    assert_eq!(
        game_row_color(RepStatus::DirInToSort),
        egui::Color32::from_rgb(40, 80, 80)
    );
    assert_eq!(
        game_row_color(RepStatus::DirUnknown),
        egui::Color32::from_rgb(60, 60, 60)
    );
    assert_eq!(
        rom_row_color(RepStatus::DirCorrect),
        egui::Color32::from_rgb(40, 80, 40)
    );
    assert_eq!(
        rom_row_color(RepStatus::DirMissing),
        egui::Color32::from_rgb(80, 40, 40)
    );
    assert_eq!(
        rom_row_color(RepStatus::DirInToSort),
        egui::Color32::from_rgb(40, 80, 80)
    );
    assert_eq!(
        rom_row_color(RepStatus::DirUnknown),
        egui::Color32::from_rgb(60, 60, 60)
    );
}
...
fn test_game_row_color_treats_needed_for_fix_and_rename_as_cyan() {
    assert_eq!(
        game_row_color(RepStatus::NeededForFix),
        egui::Color32::from_rgb(40, 80, 80)
    );
    assert_eq!(
        game_row_color(RepStatus::Rename),
        egui::Color32::from_rgb(40, 80, 80)
    );
}
...
fn test_game_and_rom_row_colors_treat_deleted_as_delete_red() {
    assert_eq!(
        game_row_color(RepStatus::Deleted),
        egui::Color32::from_rgb(120, 0, 0)
    );
    assert_eq!(
        rom_row_color(RepStatus::Deleted),
        egui::Color32::from_rgb(120, 0, 0)
    );
}
...
fn test_game_and_rom_row_colors_treat_unscanned_as_grey() {
    assert_eq!(
        game_row_color(RepStatus::UnScanned),
        egui::Color32::from_rgb(60, 60, 60)
    );
    assert_eq!(
        rom_row_color(RepStatus::UnScanned),
        egui::Color32::from_rgb(60, 60, 60)
    );
}
...
fn test_game_and_rom_row_colors_treat_ignore_as_grey() {
    assert_eq!(
        game_row_color(RepStatus::Ignore),
        egui::Color32::from_rgb(60, 60, 60)
    );
    assert_eq!(
        rom_row_color(RepStatus::Ignore),
        egui::Color32::from_rgb(60, 60, 60)
    );
}
...
fn test_game_summary_bucket_treats_directory_status_families_consistently() {
    assert_eq!(
        game_summary_bucket(RepStatus::DirCorrect),
        Some(RomStatusBucket::Correct)
    );
    assert_eq!(
        game_summary_bucket(RepStatus::DirMissing),
        Some(RomStatusBucket::Missing)
    );
    assert_eq!(
        game_summary_bucket(RepStatus::DirInToSort),
        Some(RomStatusBucket::Fixes)
    );
    assert_eq!(
        game_summary_bucket(RepStatus::DirUnknown),
        Some(RomStatusBucket::Unknown)
    );
}
...
fn test_game_summary_bucket_treats_unscanned_as_unknown() {
    assert_eq!(
        game_summary_bucket(RepStatus::UnScanned),
        Some(RomStatusBucket::Unknown)
    );
}
...
fn test_game_row_color_treats_in_to_sort_as_cyan() {
    assert_eq!(
        game_row_color(RepStatus::InToSort),
        egui::Color32::from_rgb(40, 80, 80)
    );
}
...
fn test_format_file_mod_date_cell_formats_dos_datetime() {
    ...
    assert_eq!(
        super::format_file_mod_date_cell(&node),
        "2017/01/23 15:24:36"
    );
}
...
fn test_rom_row_color_treats_missing_family_variants_as_red() {
    assert_eq!(
        rom_row_color(RepStatus::Corrupt),
        egui::Color32::from_rgb(80, 40, 40)
    );
    assert_eq!(
        rom_row_color(RepStatus::DirCorrupt),
        egui::Color32::from_rgb(80, 40, 40)
    );
    assert_eq!(
        rom_row_color(RepStatus::Incomplete),
        egui::Color32::from_rgb(80, 40, 40)
    );
}
...
fn test_rom_row_color_treats_in_to_sort_as_cyan() {
    assert_eq!(
        rom_row_color(RepStatus::InToSort),
        egui::Color32::from_rgb(40, 80, 80)
    );
}
