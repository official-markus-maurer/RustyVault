...
pub(crate) struct TaskTx {
    inner: SyncSender<String>,
}

impl TaskTx {
    pub(crate) fn send(&self, msg: String) -> Result<(), std::sync::mpsc::TrySendError<String>> {
...
pub(crate) struct QueuedTask {
    ...
    pub(crate) selection_chain: Vec<String>,
    pub(crate) task: Box<dyn FnOnce(TaskTx, ProcessControl) + Send>,
}
...
impl RomVaultApp {
    ...
    pub(crate) fn prompt_add_tosort(&mut self) {
        ...
        self.launch_task("Add ToSort Folder", move |tx, _control| {
    ...
    pub(crate) fn prompt_fix_report(&mut self) {
        ...
        self.launch_task("Generate Reports (Fix)", move |tx, _control| {
    ...
    pub(crate) fn prompt_full_report(&mut self) {
        ...
        let path_str = path.to_string_lossy().to_string();
        self.launch_task("Generate Reports (Full)", move |tx, _control| {
    ...
    pub(crate) fn prompt_make_dat(&mut self, node_rc: Rc<RefCell<RvFile>>) {
        ...
        self.launch_task("Make DAT", move |tx, _control| {
    ...
    where
        F: FnOnce(TaskTx, ProcessControl) + Send + 'static,
    {
        ...
        if self.task_running {
            self.task_queue.push_back(queued);
            self.task_logs.push(format!("Queued {}.", task_name));
            return;
        }
    ...
    fn start_task(&mut self, queued: QueuedTask) {
        let (tx_raw, rx) = sync_channel::<String>(2048);
        let tx = TaskTx {
            inner: tx_raw.clone(),
        };
        ...
        self.task_name = queued.name;
        self.task_worker_rx = Some(rx);
        self.task_selection_chain = queued.selection_chain;
        self.task_control = Some(control.clone());
        self.task_logs
            .push(format!("Starting {}...", self.task_name));

        let task = queued.task;
        self.task_worker_handle = Some(thread::spawn(move || {
            rv_core::settings::load_settings_from_file();
            rv_core::db::init_db();
            rv_core::task_reporter::set_task_reporter_bounded(tx_raw.clone());

            (task)(tx.clone(), control.clone());

            rv_core::task_reporter::clear_task_reporter();

            let cancelled = control.is_soft_stop_requested() || control.is_hard_stop_requested();
            let wrote_cache = if cancelled {
                false
            } else {
                GLOBAL_DB.with(|db_ref| {
                    db_ref
    ...
    pub(crate) fn cancel_task_soft(&mut self) {
        let Some(control) = self.task_control.as_ref() else {
            return;
        };
        control.request_soft_stop();
        self.task_logs.push("Cancel requested.".to_string());
    ...
    pub(crate) fn poll_task_worker(&mut self) {
        ...
        let wrote_cache = outcome.as_ref().is_some_and(|o| o.wrote_cache);
        ...
        let cancelled = outcome.as_ref().is_some_and(|o| o.cancelled);
        if cancelled {
            self.task_logs.push("Task cancelled.".to_string());
        } else {
            ...
            if let Some(db) = db_ref.borrow().as_ref() {
                ...
                for child in root_children {
                    if control.is_soft_stop_requested() || control.is_hard_stop_requested() {
                        let _ = tx.send("Scan cancelled.".to_string());
                        break;
                    }
    ...
    fn scan_selected_roots(
        ...
        Self::scan_roots(tx, control, scan_level, false);
    }
    ...
    fn scan_selected_roots_and_tosort(
        ...
        Self::scan_roots(tx, control, scan_level, true);
    }
    ...
    pub(crate) fn launch_scan_roms_task(
        ...
        self.launch_task(task_name, move |tx, control| {
            let _ = tx.send(status_message.to_string());
            Self::scan_selected_roots(&tx, &control, scan_level);
        });
    }

    pub(crate) fn launch_fix_roms_task(&mut self) {
        self.launch_task("Fix ROMs", |tx, control| {
            let _ = tx.send("Rescanning to refresh fix plan...".to_string());
            Self::scan_selected_roots_and_tosort(
                &tx,
                &control,
            ...
            for pass in 1..=4 {
                if control.is_soft_stop_requested() || control.is_hard_stop_requested() {
                    let _ = tx.send("Fix cancelled.".to_string());
                    break;
                }
                ...
                GLOBAL_DB.with(|db_ref| {
                    if let Some(db) = db_ref.borrow().as_ref() {
                        rv_core::task_reporter::set_task_reporter_bounded(tx.inner.clone());
    ...
    pub(crate) fn launch_scan_find_fix_fix_task(&mut self) {
        self.launch_task("Scan / Find Fix / Fix", |tx, control| {
            ...
            for pass in 1..=4 {
                if control.is_soft_stop_requested() || control.is_hard_stop_requested() {
                    let _ = tx.send("Routine cancelled.".to_string());
                    break;
                }
                let _ = tx.send(format!("Finding Fixes (pass {pass}/4)..."));
                let pending = GLOBAL_DB.with(|db_ref| {
