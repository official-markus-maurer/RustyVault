...
pub(crate) fn apply_base_dir_to_settings_paths(settings: &mut Settings, base_dir: &Path) {
    let make_absolute = |raw: &str| -> String {
    ...
    if !settings.dat_root.trim().is_empty() {
        settings.dat_root = make_absolute(&settings.dat_root);
    }
    if !settings.cache_file.trim().is_empty() {
        settings.cache_file = make_absolute(&settings.cache_file);
    }
    if let Some(p) = settings.fix_dat_out_path.clone().filter(|p| !p.trim().is_empty()) {
        settings.fix_dat_out_path = Some(make_absolute(&p));
    }

    for mapping in &mut settings.dir_mappings.items {
        if !mapping.dir_path.trim().is_empty() {
            mapping.dir_path = make_absolute(&mapping.dir_path);
        }
    }

    for rule in &mut settings.dat_rules.items {
        if let Some(dir_path) = rule.dir_path.clone().filter(|p| !p.trim().is_empty()) {
            rule.dir_path = Some(make_absolute(&dir_path));
        }
    }
}

fn resolve_settings_file_path() -> PathBuf {
    let file_name = "RomVault3cfg.xml";
...
pub fn update_settings(new_settings: Settings) {
    let mut new_settings = canonicalize_settings(new_settings);
    let settings_path = resolve_settings_file_path();
    let base_dir = settings_path
        .parent()
        .unwrap_or_else(|| Path::new(""))
        .to_path_buf();
    apply_base_dir_to_settings_paths(&mut new_settings, &base_dir);
...
pub fn write_settings_to_file(settings: &Settings) -> Result<(), Box<dyn std::error::Error>> {
    let mut settings = canonicalize_settings(settings.clone());
    let settings_path = resolve_settings_file_path();
    let base_dir = settings_path
        .parent()
        .unwrap_or_else(|| Path::new(""))
    ...
    let temp_path = path.with_extension("xml.temp");
    let backup_path = path.with_extension("xmlbackup");
