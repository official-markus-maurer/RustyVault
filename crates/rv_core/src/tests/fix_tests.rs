    ...
    fn test_fix_file_status_changes() {
        ...
        file_to_sort.borrow_mut().set_dat_got_status(dat_reader::enums::DatStatus::NotInDat, GotStatus::Got);
        ...
        assert_eq!(file_to_sort.borrow().dat_status(), dat_reader::enums::DatStatus::InToSort);
    ...
    fn test_fix_file_move_to_corrupt_marks_file_notgot() {
        ...
        file_move.borrow_mut().set_rep_status(RepStatus::MoveToCorrupt);

        Fix::fix_a_file(Rc::clone(&file_move), &mut queue, &mut total_fixed, &crc_map, &sha1_map, &md5_map);
    ...
    fn test_fix_file_move_to_corrupt_preserves_merged_source_as_not_collected() {
        ...
        let file_move = Rc::new(RefCell::new(RvFile::new(FileType::File)));
        {
            ...
            file.set_dat_got_status(dat_reader::enums::DatStatus::InDatMerged, GotStatus::Corrupt);
    ...
    fn test_fix_archive_move_to_sort_updates_dat_status_and_survives_reset() {
        ...
        assert_eq!(archive.borrow().dat_status(), dat_reader::enums::DatStatus::InToSort);
    ...
    fn test_find_source_file_matches_primary_sha1_using_alt_size() {
        ...
        let crc_map = HashMap::new();
        let mut sha1_map = HashMap::new();
        sha1_map.insert((1024, vec![0x12, 0x34, 0x56, 0x78]), Rc::clone(&source_file));
        let md5_map = HashMap::new();
    ...
    fn test_fix_file_process_queue_preserves_merged_source_as_not_collected_after_delete() {
        ...
        Fix::fix_a_file(queued_source, &mut queue, &mut total_fixed, &crc_map, &sha1_map, &md5_map);
    ...
    fn test_fix_perform_fixes_deletes_unneeded_archive() {
        ...
        assert_eq!(unneeded_archive.borrow().rep_status(), RepStatus::NotCollected);
    }
    ...
    fn test_fix_archive_rebuild_removes_unneeded_zip_member() {
        ...
        target_archive.borrow_mut().child_add(Rc::clone(&unneeded_child));
        ...
        assert_eq!(Fix::read_zip_entry_bytes(&target_path.to_string_lossy(), "keep.bin").unwrap(), b"keep");
        ...
        assert_eq!(unneeded_child.borrow().rep_status(), RepStatus::NotCollected);
    }
    ...
    fn test_fix_shared_destination_keeps_physical_file_for_other_dat_branch() {
        ...
        assert_eq!(duplicate_view.borrow().rep_status(), RepStatus::NotCollected);
    ...
    fn test_fix_zip_partial_rebuild_marks_moved_entry_in_tosort() {
        ...
        assert_eq!(moved_child.borrow().dat_status(), dat_reader::enums::DatStatus::InToSort);
    ...
    fn test_fix_zip_partial_rebuild_marks_removed_entry_missing_and_clears_got_when_archive_remains() {
        ...
        let target_path = temp.path().join("target.zip");
        {
            ...
            writer.start_file("keep.bin", SimpleFileOptions::default()).unwrap();
            writer.write_all(b"keep").unwrap();
            writer.start_file("bad.bin", SimpleFileOptions::default()).unwrap();
            writer.write_all(b"data").unwrap();
            writer.finish().unwrap();
        }
