...
fn test_phase_2_name_agnostic_test_rejects_wrong_required_header_type() {
    ...
    db_file.size = Some(4);
    db_file.crc = Some(vec![0xAD, 0xF3, 0xF3, 0x63]);
    db_file.header_file_type =
        dat_reader::enums::HeaderFileType::NES | dat_reader::enums::HeaderFileType::REQUIRED;
...
fn test_phase_2_name_agnostic_test_propagates_deep_scan_status_flags() {
    ...
    assert!(sc_file
        .status_flags
        .contains(crate::rv_file::FileStatus::HEADER_FILE_TYPE_FROM_HEADER));
    assert!(sc_file
        .status_flags
        .contains(crate::rv_file::FileStatus::ALT_CRC_FROM_HEADER));
}
