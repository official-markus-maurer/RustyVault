...
fn test_scan_raw_file_populates_headerless_alt_hashes() {
    ...
    assert!(scanned
        .status_flags
        .contains(FileStatus::HEADER_FILE_TYPE_FROM_HEADER));
    assert!(scanned
        .status_flags
        .contains(FileStatus::ALT_CRC_FROM_HEADER));
}
#[test]
fn test_scan_directory_parallel() {
    ...
    let subdir_node = scanned_children
        .iter()
        .find(|c| c.file_type == FileType::Dir)
        .unwrap();
...
fn test_scan_directory_level2_hashes_loose_files() {
    ...
    let scanned_file = scanned_children
        .iter()
        .find(|c| c.name == "hashed.bin")
        .unwrap();
...
fn test_scan_archive_file_populates_headerless_alt_hashes() {
    ...
    let scanned_archive =
        Scanner::scan_archive_file(FileType::Zip, archive_path.to_str().unwrap(), 0, true)
    ...
    assert_eq!(
        ...
        Some(crc.finalize().to_be_bytes().to_vec())
    );
    assert!(scanned_file.alt_sha1.is_some());
    assert!(scanned_file.alt_md5.is_some());
    assert!(scanned_file
        .status_flags
        .contains(FileStatus::SIZE_FROM_HEADER));
    assert!(scanned_file
        .status_flags
        .contains(FileStatus::CRC_FROM_HEADER));
    assert!(scanned_file
        .status_flags
        .contains(FileStatus::HEADER_FILE_TYPE_FROM_HEADER));
    assert!(scanned_file
...
fn test_scan_archive_file_light_scan_populates_alt_crc_for_headered_files() {
    ...
    let scanned_archive =
        Scanner::scan_archive_file(FileType::Zip, archive_path.to_str().unwrap(), 0, false)
            .expect("Failed to scan archive");
