...
fn test_new_directory_found_is_fully_integrated_recursively() {
    ...
    assert_eq!(
        subdir.children[0].borrow().dat_status(),
        dat_reader::enums::DatStatus::NotInDat
    );
}
...
fn test_match_found_updates_scanned_metadata_on_existing_file() {
    ...
    assert_eq!(
        matched.header_file_type,
        dat_reader::enums::HeaderFileType::ZIP
    );
}
...
fn test_match_found_updates_archive_zip_structure() {
    ...
    existing_archive.set_dat_got_status(
        dat_reader::enums::DatStatus::InDatCollect,
        GotStatus::NotGot,
    );
...
fn test_renamed_file_is_not_matched_during_scan_and_creates_orphan() {
    ...
    let expected = dir
        .children
        .iter()
        .find(|child| child.borrow().name == "expected.bin")
        .unwrap();
    let orphan = dir
        .children
        .iter()
        .find(|child| child.borrow().name == "renamed.bin")
        .unwrap();
    assert_eq!(expected.borrow().got_status(), GotStatus::NotGot);
    assert_eq!(
        expected.borrow().dat_status(),
        dat_reader::enums::DatStatus::InDatCollect
    );
    assert_eq!(orphan.borrow().got_status(), GotStatus::Got);
    assert_eq!(
        orphan.borrow().dat_status(),
        dat_reader::enums::DatStatus::NotInDat
    );
}
...
fn test_adjacent_file_candidate_is_realigned_before_orphaning() {
    ...
    expected.set_dat_got_status(
        dat_reader::enums::DatStatus::InDatCollect,
        GotStatus::NotGot,
    );
    db_dir
        .borrow_mut()
        .child_add(Rc::new(RefCell::new(expected)));
    ...
    let alpha = dir
        .children
        .iter()
        .find(|child| child.borrow().name == "alpha.bin")
        .unwrap();
    let target = dir
        .children
        .iter()
        .find(|child| child.borrow().name == "target.bin")
        .unwrap();
    let zzz = dir
        .children
        .iter()
        .find(|child| child.borrow().name == "zzz.bin")
        .unwrap();
    assert_eq!(
        alpha.borrow().dat_status(),
        dat_reader::enums::DatStatus::NotInDat
    );
    assert_eq!(
        zzz.borrow().dat_status(),
        dat_reader::enums::DatStatus::NotInDat
    );
...
fn test_adjacent_db_candidate_is_realigned_before_marking_missing() {
    ...
    missing.set_dat_got_status(
        dat_reader::enums::DatStatus::InDatCollect,
        GotStatus::NotGot,
    );
    db_dir
        .borrow_mut()
        .child_add(Rc::new(RefCell::new(missing)));
...
fn test_file_candidate_window_prefers_primary_match_over_nearer_alt_match() {
    ...
    let alt = dir
        .children
        .iter()
        .find(|child| child.borrow().name == "alt.bin")
        .unwrap();
    let primary = dir
        .children
        .iter()
        .find(|child| child.borrow().name == "primary.bin")
        .unwrap();
    assert_eq!(
        alt.borrow().dat_status(),
        dat_reader::enums::DatStatus::NotInDat
    );
    assert_eq!(
        primary.borrow().dat_status(),
        dat_reader::enums::DatStatus::NotInDat
    );
}
...
fn test_db_candidate_window_prefers_primary_match_over_nearer_alt_match() {
    ...
    alt_expected.set_dat_got_status(
        dat_reader::enums::DatStatus::InDatCollect,
        GotStatus::NotGot,
    );
    db_dir
        .borrow_mut()
        .child_add(Rc::new(RefCell::new(alt_expected)));
    ...
    primary_expected.set_dat_got_status(
        dat_reader::enums::DatStatus::InDatCollect,
        GotStatus::NotGot,
    );
    db_dir
        .borrow_mut()
        .child_add(Rc::new(RefCell::new(primary_expected)));
...
fn test_apply_scanned_metadata_preserves_dat_flags_and_updates_physical_flags() {
    ...
    scanned.status_flags =
        FileStatus::HEADER_FILE_TYPE_FROM_HEADER | FileStatus::ALT_CRC_FROM_HEADER;
    scanned.crc = Some(vec![1, 2, 3, 4]);
    scanned.deep_scanned = true;
    scanned_root.children.push(scanned);
