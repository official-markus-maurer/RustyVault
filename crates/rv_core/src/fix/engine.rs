impl Fix {
    ...
    fn apply_torrentzip_metadata(zip_path: &str) -> bool {
            ...
            && zip_bytes[local_offset..local_offset + 4] == local_header_signature
        {
            ...
            zip_bytes[local_offset + 10..local_offset + 12].copy_from_slice(&Self::TORRENTZIP_DOS_TIME.to_le_bytes());
            zip_bytes[local_offset + 12..local_offset + 14].copy_from_slice(&Self::TORRENTZIP_DOS_DATE.to_le_bytes());
            ...
            let file_name_length =
                u16::from_le_bytes([zip_bytes[local_offset + 26], zip_bytes[local_offset + 27]]) as usize;
            let extra_length =
                u16::from_le_bytes([zip_bytes[local_offset + 28], zip_bytes[local_offset + 29]]) as usize;

            local_offset += 30 + file_name_length + extra_length + compressed_size;
        }

        let eocd_signature = [0x50, 0x4B, 0x05, 0x06];
        let Some(eocd_offset) = zip_bytes.windows(4).rposition(|window| window == eocd_signature) else {
            return false;
        };
            ...
            && zip_bytes[central_offset..central_offset + 4] == central_header_signature
        {
            ...
            zip_bytes[central_offset + 12..central_offset + 14].copy_from_slice(&Self::TORRENTZIP_DOS_TIME.to_le_bytes());
            zip_bytes[central_offset + 14..central_offset + 16].copy_from_slice(&Self::TORRENTZIP_DOS_DATE.to_le_bytes());
        ...
        crc_hasher.update(&zip_bytes[central_directory_offset..central_directory_offset + central_directory_size]);
        let comment = format!("TORRENTZIPPED-{:08X}", crc_hasher.finalize());
        let comment_bytes = comment.into_bytes();

        zip_bytes[eocd_offset + 20..eocd_offset + 22].copy_from_slice(&(comment_bytes.len() as u16).to_le_bytes());
    ...
    fn has_selected_descendant(node: Rc<RefCell<RvFile>>) -> bool {
        ...
        for child in children {
            if Self::is_fix_selected(&child.borrow())
                || Self::has_selected_descendant(Rc::clone(&child))
            {
                return true;
            }
        }
    ...
    fn get_tosort_path(file_path: &str, base_dir: &str) -> String {
        ...
        if let Some((source_logical_key, source_root_path)) = crate::settings::find_mapping_for_physical_path(path) {
    ...
    fn read_raw_zip_entry(zip_path: &str, entry_name: &str) -> Option<StoredZipEntry> {
        ...
        let eocd_offset = zip_bytes.windows(4).rposition(|window| window == [0x50, 0x4B, 0x05, 0x06])?;
        ...
        while central_offset + 46 <= central_directory_offset + central_directory_size {
            ...
            if Self::logical_name_eq(&current_name, entry_name) {
                ...
                let local_name_length =
                    u16::from_le_bytes([zip_bytes[relative_offset + 26], zip_bytes[relative_offset + 27]]) as usize;
                let local_extra_length =
                    u16::from_le_bytes([zip_bytes[relative_offset + 28], zip_bytes[relative_offset + 29]]) as usize;
    ...
    fn find_containing_archive(file: Rc<RefCell<RvFile>>) -> Option<(Rc<RefCell<RvFile>>, String, FileType)> {
        let mut path_parts = Vec::new();
        let mut current = Some(file);
    ...
    fn read_seven_zip_entry_bytes(archive_path: &str, entry_name: &str) -> Option<Vec<u8>> {
        compress::seven_zip::extract_entry_bytes(archive_path, entry_name).ok().flatten()
    }
