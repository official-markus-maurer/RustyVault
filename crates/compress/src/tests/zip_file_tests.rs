    ...
    fn test_zip_file_detects_torrentzip_with_zip64_eocd_boundary_values() {
        ...
        let mut crc = Crc32Hasher::new();
        crc.update(
            &bytes[central_directory_offset as usize..(central_directory_offset + central_directory_size) as usize],
        );
        let comment = format!("TORRENTZIPPED-{:08X}", crc.finalize());
    ...
    fn test_zip_file_add_fake_matches_reference_header_fields_for_ascii_and_utf8_and_zstd() {
        ...
        let ascii = zip
            .zip_file_add_fake("a.bin", 0, 5, 5, &[0x36, 0x10, 0xA6, 0x86], 0, 19961224233200)
            .unwrap();
        ...
        let utf8 = zip
            .zip_file_add_fake(utf8_name, 0, 5, 5, &[0x36, 0x10, 0xA6, 0x86], 0, 19961224233200)
            .unwrap();
        ...
        let zstd = zip
            .zip_file_add_fake("zstd.bin", 0, 5, 5, &[0x36, 0x10, 0xA6, 0x86], 93, 19961224233200)
            .unwrap();
    ...
    fn test_zip_file_nonraw_deflate_empty_file_writes_torrentzip_zero_length_marker() {
        ...
        let stream = zip_file
            ...
            .unwrap();
        drop(stream);
        assert_eq!(zip_file.zip_file_close_write_stream(&[0, 0, 0, 0]), ZipReturn::ZipGood);
        zip_file.zip_file_close();
    ...
    fn test_zip_file_raw_write_stream_requires_crc() {
        ...
        assert_eq!(zip_file.zip_file_close_write_stream(&[]), ZipReturn::ZipErrorWritingToOutputStream);
