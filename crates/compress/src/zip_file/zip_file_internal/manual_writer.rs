...
impl ManualZipWriter {
    ...
    pub(crate) fn finish(&mut self, zip_struct: ZipStructure, comment: &str) -> Result<(), ZipReturn> {
        if self.finalized {
            return Ok(());
        }
        ...
        let entries_u16 = if zip64_needed { 0xFFFF } else { self.entries.len() as u16 };
        let cd_size_u32 = if zip64_needed { 0xFFFF_FFFF } else { central_directory_size as u32 };
        let cd_offset_u32 = if zip64_needed { 0xFFFF_FFFF } else { central_directory_offset as u32 };
