bitflags::bitflags! {
    ...
    ///
    /// `TrrntZipStatus` is used by the `TorrentZipCheck` engine to accumulate structural flaws found inside
    /// a standard zip file before attempting to rebuild it.
    ///
    /// Uses the `bitflags` crate to keep masking operations type-safe.
